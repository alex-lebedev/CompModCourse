function (data = "choose", niter = 3000, nwarmup = 1000, nchain = 4, 
          ...
          adapt_delta = 0.95, stepsize = 1, max_treedepth = 10) 
{
  if (modelRegressor) {
    ...
    modelPath <- system.file("stan", "bandit2arm_delta.stan", 
                             package = "hBayesDM")
  }
  startTime <- Sys.time()
  if (data == "example") {
    data <- system.file("extdata", "bandit2arm_exampleData.txt", 
                        package = "hBayesDM")
  }
  else if (data == "choose") {
    data <- file.choose()
  ...
  numPars <- 2
  POI <- c("mu_A", "mu_tau", "sigma", "A", "tau", "log_lik")
  if (inc_postpred) {
    POI <- c(POI, "y_pred")
  }
  ...
  if (inits[1] != "random") {
    ...
    genInitList <- function() {
      list(mu_p = c(qnorm(inits_fixed[1]), qnorm(inits_fixed[2]/5)), 
           sigma = c(1, 1), A_pr = rep(qnorm(inits_fixed[1]), 
                                       numSubjs), tau_pr = rep(qnorm(inits_fixed[2]/5), 
                                                               numSubjs))
    }
  }
  else {
  ...
  for (i in 1:numSubjs) {
    if (indPars == "mean") {
      allIndPars[i, ] <- c(mean(A[, i]), mean(tau[, i]))
    }
    else if (indPars == "median") {
      allIndPars[i, ] <- c(median(A[, i]), median(tau[, 
                                                      i]))
    }
    else if (indPars == "mode") {
      allIndPars[i, ] <- c(estimate_mode(A[, i]), estimate_mode(tau[, 
                                                                    i]))
    }
  }
  allIndPars <- cbind(allIndPars, subjList)
  colnames(allIndPars) <- c("A", "tau", "subjID")
